  ...
  return (
    <div className="mx-4 p-3 space-y-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-lg font-semibold">Invoice Discount Approval: {totalItems}</h1>
              ...
              {paginatedData.map((invoice) => (
                <TableRow key={invoice.id} className="hover:bg-gray-50">
                  <TableCell className="w-[80px] p-4 text-sm">
                    <img
                      src={invoice.photo}
                      alt={`${invoice.studentName}'s photo`}
                      className="w-10 h-10 rounded-full"
                    />
                  </TableCell>
                  <TableCell className="w-[120px] p-4 text-sm">{invoice.studentName}</TableCell>
                  <TableCell className="w-[100px] p-4 text-sm">{invoice.fee}</TableCell>
                  <TableCell className="w-[100px] p-4 text-sm">{invoice.month}</TableCell>
                  <TableCell className="w-[100px] p-4 text-sm">{invoice.amount}</TableCell>
                  <TableCell className="w-[100px] p-4 text-sm">{invoice.discount}</TableCell>
                  <TableCell className="w-[120px] p-4 text-sm">{invoice.payableAmount}</TableCell>
                  <TableCell className="w-[100px] p-4 text-sm">
                    <span className={invoice.status === "Pending" ? "text-red-600" : "text-green-600"}>
                      {invoice.status}
                    </span>
                  </TableCell>
